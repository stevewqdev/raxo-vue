<template :key="componentKey">
    <div class="project__block project__component section__wrapper project__internal__page" >
    ...
    <meta name="keyword" content="motion graphics portfolio, digital marketing portfolio, graphic design portfolio, professional design work, animation design portfolio">		<link rel="stylesheet" type="text/css" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css">
    <Awake v-if="['awake'].includes($route.name)"></Awake>
    <SmartCities v-else-if="['smart-cities'].includes($route.name)"></SmartCities>
    <Showtime v-else-if="['showtime'].includes($route.name)"></Showtime>
    <Allocate v-else-if="['allocate'].includes($route.name)"></Allocate>
    <Humanified v-else-if="['humanifed-branding-web-design-app-development'].includes($route.name)"></Humanified>
    <Ultra v-else-if="['ultra-music-animation-design-web-development'].includes($route.name)"></Ultra>
    <LondonCalling v-else-if="['bbc-entertainment-london-calling-design'].includes($route.name)"></LondonCalling>
    <Yankees v-else-if="['yankees-creative-design'].includes($route.name)"></Yankees>
    <Univision v-else-if="['univision-election-design-animation-tv'].includes($route.name)"></Univision>
    <theLongWay v-else-if="['the-long-way-back-entertainment-design-animation'].includes($route.name)"></theLongWay>
    <SteelBridge v-else-if="['steelbridge-labs-incubator-branding-web-design'].includes($route.name)"></SteelBridge>
    <theDepartment v-else-if="['the-department-design-web-development'].includes($route.name)"></theDepartment>
    <SimplyIraIralogix v-else-if="['simply-ira-iralogix-branding-design-web'].includes($route.name)"></SimplyIraIralogix>
    <Sambazon v-else-if="['sambazon-design-animation'].includes($route.name)"></Sambazon>
    <Quantgene v-else-if="['quantgene-is-a-cell-free-platform'].includes($route.name)"></Quantgene>
    <Pluto v-else-if="['pluto-tv-is-an-Internet-based-television'].includes($route.name)"></Pluto>
    <Oracle v-else-if="['oracle-cloud-hero-marketing-design-web-development'].includes($route.name)"></Oracle>
    <Onboard v-else-if="['onboard-informatics-design-web-development'].includes($route.name)"></Onboard>
    <Lufthansa v-else-if="['lufthansa-creative-design-web-development'].includes($route.name)"></Lufthansa>
    <LeCordonBleu v-else-if="['le-cordon-bleu-creative-design-web-development'].includes($route.name)"></LeCordonBleu>
    <Indago v-else-if="['indago-design-animation'].includes($route.name)"></Indago>
    <HugoBoss v-else-if="['hugo-boss-design-animation'].includes($route.name)"></HugoBoss>
    <GlobalPress v-else-if="['global-press-design'].includes($route.name)"></GlobalPress>
    <Fusion v-else-if="['fusion-beauty-creative-packaging'].includes($route.name)"></Fusion>
    <EntertainmentMina v-else-if="['bbc-entertainment-mina-jack-animation-design-tv'].includes($route.name)"></EntertainmentMina>
    <Coloredge v-else-if="['coloredge-is-a-unique-media-company'].includes($route.name)"></Coloredge>
    <CityCancer v-else-if="['city-cancer-challenge'].includes($route.name)"></CityCancer>
    <Cbeebies v-else-if="['bbc-entertainment-cbeebies-creative'].includes($route.name)"></Cbeebies>
    <BbcEntertainment v-else-if="['bbc-entertainment-design-animation-tv'].includes($route.name)"></BbcEntertainment>
    <BaseWeb v-else-if="['base-develops-and-manages'].includes($route.name)"></BaseWeb>
    <RockSummer v-else-if="['rock-summer-is-an-initiative-started'].includes($route.name)"></RockSummer>
    <FirstKey v-else-if="['first-key-homes-strategy-design-web-development'].includes($route.name)"></FirstKey>
    <Engine v-else-if="['engine-shop-branding-design-web-development'].includes($route.name)"></Engine>
    <Sermo v-else-if="['sermo-branding-design-web-development'].includes($route.name)"></Sermo>
    <EntertainmentHd v-else-if="['bbc-entertainment-HD-design-animation-tv'].includes($route.name)"></EntertainmentHd>
    <Investigation v-else-if="['investigation-13-entertainment-design-animation'].includes($route.name)"></Investigation>
   
    </div>
</template>

<style src="../assets/projects/css/plugins.css" ></style>
<style src="../assets/projects/css/style.css" ></style>
<style src="../assets/projects/css/bootstrap.min.css" ></style>
<script src="//script.crazyegg.com/pages/scripts/0078/3474.js"  async="async"></script>
...
<script src="../assets/projects/js/menu-js.js"></script>
<script src="../assets/projects/js/plugins.js" type="text/javascript"></script>
<script src="../assets/projects/js/script.js"></script>
<script src="../assets/projects/js/parallax.js" type="text/javascript"></script>
<style scoped>
...
export default {
  ...
  mounted(){
    ...
    var npButtons = [...document.querySelectorAll('.new-footer .nav-link')]; 
    var allElements = logo.concat(menu).concat(nextButtons).concat(bottomLinks).concat(listElements).concat(buttons).concat(aLinks).concat(inputElements).concat(otherElements).concat(npButtons);    
...
<style >
  .project__internal__page.project__component h1, .project__internal__page.project__component h2, .project__internal__page.project__component h3, .project__internal__page.project__component h4, .project__internal__page.project__component h5, .project__internal__page.project__component h6{
