<template>
    <div class="about__page page__component section__wrapper" >
    <title>About Raxo - Web & Digital Design Agency NYC, Motion Graphics Company</title>
    <meta name="description" content="Raxo is a prominent web & digital design agency in New York with a huge clientele that spreads includes Nationally & Internationally. Call motion graphics creator Call Raxo and get your motion graphics created today at 646-838-4583.">
    <meta name="keyword" content=" web design agency, creative web design, digital design agency, digital advertising agency, creative digital agency, motion graphics company">
        <section class="about-section-1">
        <div class="container">
        <div class="about-row-1 row" >
            <div class="col-md-9">
            <h1 style="color: #ffffff;" data-aos="fade-up" data-aos-delay="800" class="about__title">A small design studio big on culture</h1>
            </div>
            <div class="col-md-12">
            <h3 style="" data-aos="fade-up" data-aos-delay="850" class="about__copy">From Brooklyn to Buenos Aires, our design studio's team brings a level of cultural understanding to your brand that you won’t find anywhere else. Whether it’s branding for a startup or an Emmy-worthy creative campaign for primetime TV, we’ve got you. Our agency's expert creatives and strategists will bring your brand to life using the passion and precision we’ve won lots of awards for in design and creative. We know what’s coming, and we’ll take you there.</h3>
            </div>
        <!--     <div class="col-md-6">
                <center>
                <div class = "about-big-circle"></div>
                </center>
            </div>-->
        </div>
        </div>
        </section>
        <section>
        <div class="container-fluid" data-aos="fade-up" data-aos-delay="900">
            <div class="col-md-12 fondo1 bgs">

            </div>
        </div>
        </section>
        <section class="about-section-2">
            <div class="container">
                <h4 style="color:#fff" data-aos="fade-up" data-aos-delay="100">SERVICES</h4>
            </div>
            <div class="container-fluid">
                        <div class="about-row-1 row fondo2 bgs"></div>
                        <div class="about-row-1 row fondo3 bgs"></div>
                        <div class="about-row-1 row fondo4 bgs"></div>
            </div>

            <div class="container service-cols">
                <div class="col-xs-12 col-md-4">
                    <ul>
                        <li data-aos="fade-up" data-aos-delay="200">Brand Identity</li>
                        <li data-aos="fade-up" data-aos-delay="300">Brand Guidelines</li>
                        <li data-aos="fade-up" data-aos-delay="400">Brand Mesaging</li>
                        <li data-aos="fade-up" data-aos-delay="500">Voice & Tone</li>
                        <li data-aos="fade-up" data-aos-delay="600">UX/UI Design</li>
                        <li data-aos="fade-up" data-aos-delay="700">Mobile & App Development</li>
                        <li data-aos="fade-up" data-aos-delay="800">Web Design & Development</li>
                    </ul>
                </div>
                <div class="col-xs-12 col-md-4">
                    <ul>
                        <li data-aos="fade-up" data-aos-delay="200">Research</li>
                        <li data-aos="fade-up" data-aos-delay="300">Brand Strategy & Positioning</li>
                        <li data-aos="fade-up" data-aos-delay="400">Content Strategy & Creation</li>
                        <li data-aos="fade-up" data-aos-delay="500">Digital Strategy</li>
                        <li data-aos="fade-up" data-aos-delay="600">Email Marketing</li>
                        <li data-aos="fade-up" data-aos-delay="700">SEO / SEM</li>
                        <li data-aos="fade-up" data-aos-delay="800">Social Media</li>
                    </ul>
                </div>
                <div class="col-xs-12 col-md-4">
                    <ul>
                        <li data-aos="fade-up" data-aos-delay="200">Motion Graphics</li>
                        <li data-aos="fade-up" data-aos-delay="300">Animation</li>
                        <li data-aos="fade-up" data-aos-delay="400">Title Sequences</li>
                        <li data-aos="fade-up" data-aos-delay="500">Character Design</li>
                        <li data-aos="fade-up" data-aos-delay="600">Ad Campaigns</li>
                        <li data-aos="fade-up" data-aos-delay="700">Copywriting</li>
                        <li data-aos="fade-up" data-aos-delay="800">Art Direction</li>
                        <li data-aos="fade-up" data-aos-delay="900">Video Production</li>
                    </ul>
                </div>
            </div>
            
            <div class="container text-new" data-aos="fade-up" data-aos-delay="200">
                <p>Specializing in design, branding, motion graphics, and animation, we’re a creative agency offering startups and entertainment companies services that integrate design into every layer of your brand. </p>
            </div>
            
        </section>
        <section style="padding: 4% 0 0 0;">
            <div class="container">
            <h4 class = "about-head-text" data-aos="fade-up" data-aos-delay="200">AWARDS</h4>
                <div class="about-row-2 row">
                <div class="col-md-6">
                    <div class="row">
                    <div class="col-md-1">
                        <img :data-src="require('@/assets/team/img/darrow.png')" alt="logo" class="lazyload img-responsive" data-aos-anchor-placement="top-bottom" data-aos="fade-up" data-aos-delay="100" />
                    </div>
                    <div class="about-cols-1 col-md-5 col-md-offset-0 col-xs-offset-1">
                        <h3 data-aos="fade-up" data-aos-delay="300">
                        Emmy&copy;<br>Awards 
                        </h3>
                    </div>
                    <div class="col-md-2 col-xs-3">
                    <h1 class = "about-bar" data-aos="fade-right" data-aos-delay="100">/</h1>
                    </div>
                    <div class="col-md-4 col-xs-6">
                    <h1 class = "about-number" data-aos="fade-up" data-aos-delay="400">09</h1>
                    </div>
                    </div>
                </div>
                <div class="col-md-6">
                    <div class="row">
                    <div class="col-md-1">
                        <img :data-src="require('@/assets/team/img/darrow.png')"  alt="logo" class="lazyload img-responsive" data-aos-anchor-placement="top-bottom"  data-aos="fade-up" data-aos-delay="100" />
                    </div>
                    <div class="about-cols-1 col-md-5 col-md-offset-0 col-xs-offset-1">
                        <h3 data-aos="fade-up" data-aos-delay="200">Awwwards
                        </h3>
                    </div>
                    <div class="col-md-2 col-xs-3">
                    <h1 class = "about-bar" data-aos="fade-right" data-aos-delay="200">/</h1>
                    </div>
                    <div class="col-md-4 col-xs-6">
                    <h1 class = "about-number" data-aos="fade-up" data-aos-delay="300">03</h1>
                    </div>
                    </div>
                </div>
                </div>
                <div class="about-row-3 row">
                <div class="col-md-6">
                    <div class="row">
                    <div class="col-md-1 ">
                        <img :data-src="require('@/assets/team/img/darrow.png')" alt="logo" class="lazyload img-responsive" data-aos-anchor-placement="top-bottom" data-aos="fade-up" data-aos-delay="100" />
                    </div>
                    <div class="about-cols-1 col-md-5 col-md-offset-0 col-xs-offset-1">
                        <h3 data-aos="fade-up" data-aos-delay="400">
                        Css Design<br>Awards 
                        </h3>
                    </div>
                    <div class="col-md-2 col-xs-3">
                    <h1 class = "about-bar" data-aos="fade-right" data-aos-delay="300">/</h1>
                    </div>
                    <div class="col-md-4 col-xs-6">
                    <h1 class = "about-number" data-aos="fade-up" data-aos-delay="500">04</h1>
                    </div>
                    </div>
                </div>
                <div class="col-md-6">
                    <div class="row">
                    <div class="col-md-1">
                        <img :data-src="require('@/assets/team/img/darrow.png')"  alt="logo" class="lazyload img-responsive" data-aos-anchor-placement="top-bottom" data-aos="fade-up" data-aos-delay="100" />
                    </div>
                    <div class="about-cols-1 col-md-5 col-md-offset-0 col-xs-offset-1">
                        <h3 data-aos="fade-up" data-aos-delay="200">Promax<br>
                        Dba
                        </h3>
                    </div>
                    <div class="col-md-2 col-xs-3">
                    <h1 class = "about-bar" data-aos="fade-right" data-aos-delay="400">/</h1>
                    </div>
                    <div class="col-md-4 col-xs-6">
                    <h1 class = "about-number" data-aos="fade-up" data-aos-delay="300">01</h1>
                    </div>
                    </div>
                </div>
                </div>
            </div>
        </section>
        <section class = "about-section-3">
        <div class="container">
            <h4 style="color:#fff" data-aos="fade-up" data-aos-delay="200">SOME OF OUR CLIENTS</h4>
            <div class="about-row-4 first row">
                <div class="col-xs-6 col-md-2">
                <a href = "#" >
                    <picture>
                        <source :data-src="require('@/assets/team/img/bbc.webp')" type="image/webp"> 
                        <source :data-src="require('@/assets/team/img/bbc.png')" type="image/jpeg"> 
                        <img data-aos="fade-up" data-aos-delay="100" :data-src="require('@/assets/team/img/bbc.png')" alt="logo" class = "lazyload about-logo img-responsive" width = "100px;"/>
                    </picture>
                </a>
                </div>
                <div class="col-xs-6 col-md-2">
                <a href = "#" >
                    <picture>
                        <source :data-src="require('@/assets/team/img/hbo.webp')" type="image/webp"> 
                        <source :data-src="require('@/assets/team/img/hbo.png')" type="image/jpeg"> 
                        <img data-aos="fade-up" data-aos-delay="200" :data-src="require('@/assets/team/img/hbo.png')" alt="logo" class = "lazyload about-logo img-responsive" width = "100px;"/>
                    </picture>
                </a>
                </div>
                <div class="col-xs-6 col-md-2">
                <a href = "#" >
                    <picture>
                        <source :data-src="require('@/assets/team/img/coloredge.webp')" type="image/webp"> 
                        <source :data-src="require('@/assets/team/img/coloredge.png')" type="image/jpeg"> 
                        <img data-aos="fade-up" data-aos-delay="300" :data-src="require('@/assets/team/img/coloredge.png')"  alt="logo" class = "lazyload about-logo img-responsive" width = "100px;"/>
                    </picture>

                </a>
                </div>
                <div class="col-xs-6 col-md-2">
                <a href = "#">
                    <picture>
                        <source :data-src="require('@/assets/team/img/ultramusic.webp')" type="image/webp"> 
                        <source :data-src="require('@/assets/team/img/ultramusic.png')" type="image/jpeg"> 
                        <img  data-aos="fade-up" data-aos-delay="400" :data-src="require('@/assets/team/img/ultramusic.png')"  alt="logo" class = "lazyload about-logo img-responsive" width = "100px;"/>
                    </picture>

                </a>
                </div>
                <div class="col-xs-6 col-md-2">
                    <a href = "#" >
                        <picture>
                            <source :data-src="require('@/assets/team/img/hilton.webp')" type="image/webp"> 
                            <source :data-src="require('@/assets/team/img/hilton.png')" type="image/jpeg"> 
                            <img  data-aos="fade-up" data-aos-delay="500" :data-src="require('@/assets/team/img/hilton.png')"  alt="logo" class = "lazyload about-logo img-responsive" width = "100px;"/>
                        </picture>
                    </a>
                </div>
                <div class="col-xs-6 col-md-2">
                    <a href = "#" >
                        <picture>
                            <source :data-src="require('@/assets/team/img/starwood.webp')" type="image/webp"> 
                            <source :data-src="require('@/assets/team/img/starwood.png')" type="image/jpeg"> 
                            <img data-aos="fade-up" data-aos-delay="600" :data-src="require('@/assets/team/img/starwood.png')"  alt="logo" class = "lazyload about-logo img-responsive" width = "100px;"/>
                        </picture>

                    </a>
                </div>
                </div>
                <div class="about-row-4 row">
                <div class="col-xs-6 col-md-2">
                    <a href = "#" >
                        <picture>
                            <source :data-src="require('@/assets/team/img/cole-haan.webp')" type="image/webp"> 
                            <source :data-src="require('@/assets/team/img/cole-haan.png')" type="image/jpeg"> 
                            <img data-aos="fade-up" data-aos-delay="100" :data-src="require('@/assets/team/img/cole-haan.png')" alt="logo" class = "lazyload about-logo img-responsive" width = "100px;" style = "margin-top:20px;"/>
                        </picture>

                    </a>
                </div>
                <div class="col-xs-6 col-md-2">
                    <a href = "#" >
                        <picture>
                            <source :data-src="require('@/assets/team/img/oracle.webp')" type="image/webp"> 
                            <source :data-src="require('@/assets/team/img/oracle.png')" type="image/jpeg"> 
                            <img data-aos="fade-up" data-aos-delay="200" :data-src="require('@/assets/team/img/oracle.png')"  alt="logo" class = "lazyload about-logo img-responsive" width = "100px;" style = "margin-top:15px;"/>
                        </picture>

                    </a>
                </div>
                <div class="col-xs-6 col-md-2">
                    <a href = "#" >
                        <picture>
                            <source :data-src="require('@/assets/team/img/sales-force.webp')" type="image/webp"> 
                            <source :data-src="require('@/assets/team/img/sales-force.png')" type="image/jpeg"> 
                            <img data-aos="fade-up" data-aos-delay="300" :data-src="require('@/assets/team/img/sales-force.png')" alt="logo" class = "lazyload about-logo img-responsive" width = "100px;"/>
                        </picture>

                    </a>
                </div>
                    <div class="col-xs-6 col-md-2">
                    <a href = "#" >
                        <picture>
                            <source :data-src="require('@/assets/team/img/smartcities.webp')" type="image/webp"> 
                            <source :data-src="require('@/assets/team/img/smartcities.png')" type="image/jpeg"> 
                            <img data-aos="fade-up" data-aos-delay="400" :data-src="require('@/assets/team/img/smartcities.png')" alt="logo" class = "lazyload about-logo img-responsive" width = "100px;"/>
                        </picture>

                    </a>
                </div>
                <div class="col-xs-6 col-md-2">
                    <a href = "#" >
                        <picture>
                            <source :data-src="require('@/assets/team/img/showtime.webp')" type="image/webp"> 
                            <source :data-src="require('@/assets/team/img/showtime.png')" type="image/jpeg"> 
                            <img data-aos="fade-up" data-aos-delay="500" :data-src="require('@/assets/team/img/showtime.png')"       alt="logo" class = "lazyload about-logo img-responsive" width = "100px;"/>
                        </picture>

                    </a>
                </div>
                <div class="col-xs-6 col-md-2">
                    <a href = "#" >
                        <picture>
                            <source :data-src="require('@/assets/team/img/lecordonbleu.webp')" type="image/webp"> 
                            <source :data-src="require('@/assets/team/img/lecordonbleu.png')" type="image/jpeg"> 
                            <img data-aos="fade-up" data-aos-delay="600" :data-src="require('@/assets/team/img/lecordonbleu.png')"  alt="logo" class = "lazyload about-logo img-responsive" width = "100px;"/>
                        </picture>

                    </a>
                </div>
                </div>
                <div class="about-row-5 row">
                    <div class="col-xs-6 col-md-2">
                        <a href = "#" >
                            <picture>
                                <source :data-src="require('@/assets/team/img/discovery-channel.webp')" type="image/webp"> 
                                <source :data-src="require('@/assets/team/img/discovery-channel.png')" type="image/jpeg"> 
                                <img data-aos="fade-up" data-aos-delay="100" :data-src="require('@/assets/team/img/discovery-channel.png')" alt="logo" class = "lazyload about-logo img-responsive" width = "100px;"/>
                            </picture>

                        </a>
                        </div>
                        <div class="col-xs-6 col-md-2">
                        <a href = "#" >
                            <picture>
                                <source :data-src="require('@/assets/team/img/univision.webp')" type="image/webp"> 
                                <source :data-src="require('@/assets/team/img/univision.png')" type="image/jpeg"> 
                                <img data-aos="fade-up" data-aos-delay="200" :data-src="require('@/assets/team/img/univision.png')"  alt="logo" class = "lazyload about-logo img-responsive" width = "100px;" />
                            </picture>
                        </a>
                        </div>
                        <div class="col-xs-6 col-md-2">
                        <a href = "#" >
                            <picture>
                                <source :data-src="require('@/assets/team/img/epson.webp')" type="image/webp"> 
                                <source :data-src="require('@/assets/team/img/epson.png')" type="image/jpeg"> 
                                <img data-aos="fade-up" data-aos-delay="300" :data-src="require('@/assets/team/img/epson.png')"  alt="logo" class = "lazyload about-logo img-responsive" width = "100px;"/>
                            </picture>

                        </a>
                    </div>
                    <div class="col-xs-6 col-md-2">
                        <a href = "#" >
                            <picture>
                                <source :data-src="require('@/assets/team/img/benq.webp')" type="image/webp"> 
                                <source :data-src="require('@/assets/team/img/benq.png')" type="image/jpeg"> 
                                <img data-aos="fade-up" data-aos-delay="400" :data-src="require('@/assets/team/img/benq.png')"  alt="logo" class = "lazyload about-logo img-responsive" width = "100px;" />
                            </picture>
                        </a>
                        </div>
                        <div class="col-xs-6 col-md-2">
                        <a href = "#" >
                            <picture>
                                <source :data-src="require('@/assets/team/img/benq.webp')" type="image/webp"> 
                                <source :data-src="require('@/assets/team/img/benq.png')" type="image/jpeg"> 
                                <img data-aos="fade-up" data-aos-delay="500" :data-src="require('@/assets/team/img/adata.png')"  alt="logo" class = "lazyload about-logo img-responsive" width = "100px;"/>
                            </picture>

                        </a>
                    </div>
                    <div class="col-xs-6 col-md-2">
                    <a href = "#" >
                        <picture>
                            <source :data-src="require('@/assets/team/img/cebeebies.webp')" type="image/webp"> 
                            <source :data-src="require('@/assets/team/img/cebeebies.png')" type="image/jpeg"> 
                            <img data-aos="fade-up" data-aos-delay="600" :data-src="require('@/assets/team/img/cebeebies.png')"  alt="logo" class = "lazyload about-logo img-responsive" width = "100px;" />
                        </picture>

                    </a>
                    </div>
                </div>
                <div class="about-row-5 row" style="margin-bottom: 40px;">
                    <div class="col-xs-6 col-md-2">
                        <a href = "#" >
                            <picture>
                                <source :data-src="require('@/assets/team/img/the-hal-guys.webp')" type="image/webp"> 
                                <source :data-src="require('@/assets/team/img/the-hal-guys.png')" type="image/jpeg"> 
                                <img data-aos="fade-up" data-aos-delay="100" :data-src="require('@/assets/team/img/the-hal-guys.png')" alt="logo" class = "lazyload about-logo img-responsive" width = "100px;"/>
                            </picture>
                        </a>
                    </div>
                    <div class="col-xs-6 col-md-2">
                        <a href = "#" >
                            <picture>
                                <source :data-src="require('@/assets/team/img/bbdo.webp')" type="image/webp"> 
                                <source :data-src="require('@/assets/team/img/bbdo.png')" type="image/jpeg"> 
                                <img data-aos="fade-up" data-aos-delay="200" :data-src="require('@/assets/team/img/bbdo.png')"  alt="logo" class = "lazyload about-logo img-responsive" width = "100px;" />
                            </picture>

                        </a>
                        </div>
                    <div class="col-xs-6 col-md-2">
                        <a href = "#" >
                            <picture>
                                <source :data-src="require('@/assets/team/img/ogi.webp')" type="image/webp"> 
                                <source :data-src="require('@/assets/team/img/ogi.png')" type="image/jpeg"> 
                                <img data-aos="fade-up" data-aos-delay="300" :data-src="require('@/assets/team/img/ogi.png')" alt="logo" class = "lazyload about-logo img-responsive" width = "100px;"/>
                            </picture>

                        </a>
                    </div>
                    <div class="col-xs-6 col-md-2">
                        <a href = "#" >
                            <picture>
                                <source :data-src="require('@/assets/team/img/lufthansa.webp')" type="image/webp"> 
                                <source :data-src="require('@/assets/team/img/lufthansa.png')" type="image/jpeg"> 
                            <img data-aos="fade-up" data-aos-delay="400" :data-src="require('@/assets/team/img/lufthansa.png')"  alt="logo" class = "lazyload about-logo img-responsive" width = "100px;" />
                            </picture>

                        </a>
                    </div>
                    <div class="col-xs-6 col-md-2">
                        <a href = "#" >
                            <picture>
                                <source :data-src="require('@/assets/team/img/humanified.webp')" type="image/webp"> 
                                <source :data-src="require('@/assets/team/img/humanified.png')" type="image/jpeg"> 
                            <img data-aos="fade-up" data-aos-delay="500" :data-src="require('@/assets/team/img/humanified.png')"  alt="logo" class = "lazyload about-logo img-responsive" width = "100px;"/>
                            </picture>

                        </a>
                    </div>
                    <div class="col-xs-6 col-md-2">
                        <a href = "#" >
                            <picture>
                                <source :data-src="require('@/assets/team/img/new-york.webp')" type="image/webp"> 
                                <source :data-src="require('@/assets/team/img/new-york.png')" type="image/jpeg"> 
                                <img data-aos="fade-up" data-aos-delay="600" :data-src="require('@/assets/team/img/new-york.png')"  alt="logo" class = "lazyload about-logo img-responsive" width = "100px;" />
                            </picture> 
                        </a>
                    </div>
                </div>
        </div>
        </section>

    </div>
</template>

<style scoped>
@media(max-width: 4000px){
    .about-section-2 .container, .about-section-2 .container-fluid{
        width: 50%!important;
    }
}
    body{
        font-family: MarkPro-Book;
    }
    .service-cols ul {
        list-style: none;
    }
    .service-cols ul li, .text-new p {
        color: #fff;
        font-size: 2rem;
    }
    .text-new p {
        margin: 50px auto 100px;
        font-family: MarkPro-Book;
    }
    .service-cols ul{
            text-align: left;
            font-family: MarkPro-Book;
    }
    .about-section-2 .container{
        text-align: left;
        font-family: MarkPro-Book;
    }
    .about-row-1{
        text-align: left;
        font-family: MarkPro-Book;
    }
    #teampage img{
    width:100%;
}
.cols{
	margin: 0 !important;
	background-color:black;
	color:white;
}
.cols h1{
	writing-mode: vertical-lr; 
    transform: rotate(180deg);
	font-size:9vw;
	color:#808080;
}
.span{
	height:25px;
	width:25px;
	background-color:transparent;
	border:1px solid #111111;
	border-radius:50%;
	float:right;
	margin:20px 20px 0px 0px;
}
.cols-1 h1{
	margin-top:100px;
	letter-spacing:5px;
}
.links{
	padding:5px;
	color:#111111;
}
.links:focus{
	text-decoration:none;
	color:#111111;
}
.links:hover{
	text-decoration:none;
	color:#111111;
}
.cols-2{
	margin-top:70px;
}
.fa{
	font-size:27px !important;
	margin-top:-20px;
}
.fa-1{
	color:red;
}


/* About */
.about-section-1{
    background-color:black;
        color: white;
        padding: 5% 0
}
.about-circle{
	height:25px;
	width:25px;
	background-color:transparent;
	border:1px solid #111111;
	border-radius:50%;
	float:right;
	margin:20px 0px 0px 0px;
}
.about-big-circle{
	height:170px;
	width:170px;
	background-color:transparent;
	border:3px solid #111111;
	border-radius:50%;
	margin-top:7%;
}
.about-row-1{
    margin: 80px 10px 30px;
}
.about-section-2 .about-row-1{
	margin-top:0%;
	margin-bottom:0%;
}
@media(max-width: 2000px){
.about-section-2 .container{
    width: 85.5%!important;
}
}

.about-text-1{
	font-size:38px;
	line-height:60px;
	margin-top:10%;
	letter-spacing:3px;
}
.about-big-circle-red{
	height:170px;
	width:170px;
	background-color:transparent;
	border:4px solid #FF0067;
	border-radius:50%;
	margin-top:7%;
	margin-bottom:10%;
	position:relative;
	margin-top:-21%;
	margin-left:9%;
}
.about-section-2{
	background-color:black;
        padding-top: 6%;
}
.about-section-2 h6{
	color:#fff;
	margin-top:50px;
	margin-bottom:15px;
}
.about-section-2 h4{
	color:#fff;
}
.about-row-2{
	margin-top:5%;
}
.about-cols-1 h3{
	margin-top:0px !important;
        text-transform: uppercase;
            font-family: MarkPro;
}
h1, h2, h3, h4, h5, h6{
        font-family: MarkPro;
}
.about-bar{
	font-size:140px;
	margin-top:-20px;
	color:#111111;
}
.about-head-text{
	margin-top:50px;
}
.about-number{
	font-size:140px;
	margin-top:-20px;
    color:#111111;
    font-family: MarkPro;
}
.about-row-3{
	margin-top:3%;
	margin-bottom:10%; 
}
.about-section-3{
	background-color:black;
        padding: 6% 0;
}
.about-section-3 h6{
	margin-top:5%;
	margin-bottom:5%;
	color:#fff;
}
.about-row-4.first{
	margin-top:70px;
}
/*.about-row-5{
	margin-top:50px;
	margin-bottom:10%;
}*/
.about-logo{
	margin:auto;
	display:block;
}


/*Featured Projects */
.pro-cols-1{
	padding-left:7.5px !important;
	padding-right:7.5px !important;
}
.pro-cols-1 img{
	height:583px;
}
.pro-cols-2{
	padding-left:7.5px !important;
	padding-right:7.5px !important;
}
.pro-cols-2 img{
	height:583px;
}
.pro-cols-3{
	padding-left:7.5px !important;
	padding-right:0px !important;
	margin-bottom:10%;
}
.pro-cols-3 img{
	height:583px;
}
.pro-cols-4 img{
	margin-top:150%;
}
.pro-cols-4{
	padding:0px !important;
}
.pro-cols-4 img, h2, h6, a{
	margin-left:0px;
}
.pro-cols-5{
	padding-left:30px !important;
}

/*End */


/*Team Start */
.team-row-1{
	margin-top:10%;
	margin-bottom:10%;
}
.team-row-1 h1{
	letter-spacing:5px;
	word-spacing:5px;
	color:#111111;
	font-size:50px;
        font-weight: bold;
}
.team-cols-1{
	padding:0px !important;
}
.team-cols-2{
	padding:0px !important;
	position: relative;
}	
.team-img{
  display: block;
  width: 100%;
  height: auto;
}
.team-overlay{
  position: absolute;
  bottom: 100%;
  left: 0;
  right: 0;
  background-color: #111111;
  overflow: hidden;
  width: 100%;
  height:0;
  transition: .5s ease;
}
.team-cols-2:hover .team-overlay{
	bottom: 0;
    height: 100%;
}
.team-text-1{
  color: #fff;
  position: absolute;
  top: 85%;
  left: 37%;
  transform: translate(-50%, -50%);
  -ms-transform: translate(-50%, -50%);
  text-align: left;
  font-size:17px;
}
.team-last-row{
	margin-bottom:10%;
}


.menu-circle{
	height:50px;
	width:50px;
	background-color:transparent;
	border:1px solid #fff;
	border-radius:50%;
	float:right;
	margin:20px 0px 0px 30px;
	transition: 1s;
}
.menu-circle:hover{
	background-color:#fff;
	transition:.5s;
	text-decoration:none;
}
.menu-circle:active{
	background-color:#fff;
	text-decoration:none;
}
.menu-circle:focus{
	outline:none !important;
	text-decoration:none;
}
.menu-circle-active{
	background-color:#fff;
	text-decoration:none;
}
.contact-circle{
	height:50px;
	width:50px;
	background-color:transparent;
	border:2px solid #3BEAD0;
	border-radius:50%;
	float:right;
	margin:20px 0px 0px 20px;
	transition: 1s;
}
.contact-circle:hover{
	background-color:#3BEAD0;
	transition: 1s;
	text-decoration:none;
}
.contact-circle:active{
	background-color:#3BEAD0;
	text-decoration:none;
}
.contact-circle:focus{
	outline:none !important;
	text-decoration:none;
}
.contact-circle-active{
	background-color:#3BEAD0;
	text-decoration:none;
}
.contact-text{
    color: #fff;
    display: none;
    transition: 1s;
    text-align: center;
    margin-top: 33px;
    font-size: 14pt;
    margin-left: -9px;
	
}
.about-circle{
	height:50px;
	width:50px;
	background-color:transparent;
	border:2px solid #F00B57;
	border-radius:50%;
	float:right;
	margin:20px 0px 0px 20px;
	transition: 1s;
}
.about-circle:hover{
	background-color:#F00B57;
    transition: 1s;
    text-decoration:none;
}
.about-circle:active{
	background-color:#F00B57;
	text-decoration:none;
}
.about-circle:focus{
	outline:none !important;
	text-decoration:none;
}
.about-circle-active{
	background-color:#F00B57;
	text-decoration:none;
}
.about-text {
    color: #fff;
    transition: 1s;
    text-align: center;
    margin-top: 33px;
    font-size: 14pt;
}
.work-circle{
	height:50px;
	width:50px;
	background-color:transparent;
	border:2px solid #FBAE17;
	border-radius:50%;
	float:right;
	margin:20px 0px 0px 20px;
	transition: 1s;
}
.work-circle:hover{
	background-color:#FBAE17;
	text-decoration:none;
	transition: 1s;
}
.work-circle:active{
	background-color:#FBAE17;
	text-decoration:none;	
}
.work-circle:focus{
	outline:none !important;
	text-decoration:none;
}
.work-circle-active{
	background-color:#FBAE17;
	text-decoration:none;	
}
.work-text{
    color: #fff;
    display: none;
    transition: 1s;
    text-align: center;
    margin-top: 33px;
    font-size: 14pt;
}
.team-circle{
	height:50px;
	width:50px;
	background-color:transparent;
	border:2px solid #151BBD;
	border-radius:50%;
	float:right;
	margin:20px 0 0 20px;
	transition: 1s;
}
.team-circle:hover{
	background-color:#151BBD;
	transition: 1s;
	text-decoration:none;
}
.team-circle:active{
	background-color:#151BBD;
	text-decoration:none;
}
.team-circle:focus{
	outline:none !important;
	text-decoration:none;
}
.team-circle-active{
	background-color:#151BBD;
	text-decoration:none;
}
.team-text{
    color: #fff;
    display: none;
    transition: 1s;
    text-align: center;
    margin-top: 33px;
    font-size: 14pt;
}
.center-container {
    position: absolute;
    display: table;
    table-layout: fixed;
    width: 100%;
    height: 100%;
    background: none;
}
.center-block {
    display: table-cell !important;
    vertical-align: middle;
}



.main-navigation-logo {
	float: left;
}
.main-navigation-logo .main-navigation-logo-img {
	display: table-cell;
	vertical-align: middle;
	max-width: 200px;
	height: 75px;
	-webkit-transition: all 0.27s cubic-bezier(0.000, 0.000, 0.580, 1.000);
	-moz-transition: all 0.27s cubic-bezier(0.000, 0.000, 0.580, 1.000);
	-ms-transition: all 0.27s cubic-bezier(0.000, 0.000, 0.580, 1.000);
	-o-transition: all 0.27s cubic-bezier(0.000, 0.000, 0.580, 1.000);
	transition: all 0.27s cubic-bezier(0.000, 0.000, 0.580, 1.000);
}
#grid-logo, .logo-footer img {
    width: 74px;
}
.reduce-height #grid-logo .cls-1, .reduce-height #grid-logo .cls-2 {
    fill: #fff;
}
.reduce-height #menu-btnx .st0 {
    stroke: #fff;
}
@media (max-width: 991px){
    .main-navigation-logo .main-navigation-logo-img {
        height: 60px;
    }
}
@media (min-width: 991px){
    .redes{
        position: absolute;
        bottom: 5%;
        right: 5%;
        text-align: right;
        font-size: 17px;
    }
    .heightcontact {
        height: 100%;
    }
}
.main-navigation-logo .main-navigation-logo-img img {
	max-width: 100%;
	max-height: 100%;
	height: auto;
        width: 74px;
}
.mobile .reduce-height .main-navigation-logo .main-navigation-logo-img img {
        margin-top: 12px; /*JLMA*/
}
.desktop #menu-btnx { 
    margin-top: 20px;
}
.main-navigation-logo .main-navigation-logo-img:before, .main-navigation-logo .main-navigation-logo-img:after {
	display: none;
}
.main-navigation-logo-img, a.main-navigation-logo-img:hover {
	cursor: pointer;
}
.main-navigation {
	position: relative;
	top: 0;
	left: 0;
	text-align: left;
	width: 100%;
	/*height: 75px!important; JLMA*/
	-webkit-transition: height 0.27s cubic-bezier(0.000, 0.000, 0.580, 1.000);
	-moz-transition: height 0.27s cubic-bezier(0.000, 0.000, 0.580, 1.000);
	-ms-transition: height 0.27s cubic-bezier(0.000, 0.000, 0.580, 1.000);
	-o-transition: height 0.27s cubic-bezier(0.000, 0.000, 0.580, 1.000);
	transition: height 0.27s cubic-bezier(0.000, 0.000, 0.580, 1.000);
	background-color: rgba(17,  17,  17,  .5);
	z-index: 9;
}

.main-navigation .main-inner-navigation ul > li > a {
	color: #fff;
}
.main-navigation .main-inner-navigation ul > li > a:hover {
	color: rgba(255,  255,  255,  1);
}
.main-navigation .main-inner-navigation ul > li > a.active {
	color: rgba(255,  255,  255,  .55);
}
.reduce-height {
	/*height: 110px!important; JLMA*/
	/*padding: 0 18px; JLMA*/
}
.reduce-height .main-inner-navigation ul li a {
	height: 55px!important;
	line-height: 55px!important;
}
.main-inner-navigation {
	position: relative;
	display: inline-block;
	float: right;
}
.main-inner-navigation ul {
	font-style: normal;
	font-weight: 400;
	font-size: 11px;
	text-align: center;
	text-transform: uppercase;
	letter-spacing: 2px;
	float: right;
	margin: auto;
	text-shadow: 1px 1px 2px #111;
}
.main-inner-navigation ul li {
	float: left;
	margin-left: 25px;
	position: relative;
}
.main-inner-navigation ul li a {
	display: inline-block;
	text-decoration: none;
	-webkit-box-sizing: border-box;
	-moz-box-sizing: border-box;
	box-sizing: border-box;
	-webkit-transition: all 0.27s cubic-bezier(0.000, 0.000, 0.580, 1.000);
	-moz-transition: all 0.27s cubic-bezier(0.000, 0.000, 0.580, 1.000);
	-ms-transition: all 0.27s cubic-bezier(0.000, 0.000, 0.580, 1.000);
	-o-transition: all 0.27s cubic-bezier(0.000, 0.000, 0.580, 1.000);
	transition: all 0.27s cubic-bezier(0.000, 0.000, 0.580, 1.000);
}
.main-inner-navigation ul > li:first-child > a {
}
.main-inner-navigation ul li a:hover, .main-inner-navigation ul li a.active {
	color: #fff;
	text-decoration: none;
}
.main-navigation.transparent {
	background: transparent!important;
	/*padding: 0 25px; JLMA*/
}
.main-inner-navigation .menu-link {
	font-size: 12px;
	text-shadow: none;
}
.main-navigation.js-transparent {
	-webkit-transition: all 0.27s cubic-bezier(0.000, 0.000, 0.580, 1.000);
	-moz-transition: all 0.27s cubic-bezier(0.000, 0.000, 0.580, 1.000);
	-ms-transition: all 0.27s cubic-bezier(0.000, 0.000, 0.580, 1.000);
	-o-transition: all 0.27s cubic-bezier(0.000, 0.000, 0.580, 1.000);
	transition: all 0.27s cubic-bezier(0.000, 0.000, 0.580, 1.000);
}
.main-navigation-logo {
    float: left;
    margin-left: 2vw;
}
.main-inner-navigation {
    position: relative;
    display: inline-block;
    float: right;
    margin-right: 25px;
}
@media (max-width: 991px){
    .main-navigation-logo .main-navigation-logo-img {
        height: 60px;
    }
}
.main-navigation-logo .main-navigation-logo-img img {
	max-width: 100%;
	max-height: 100%;
	height: auto;
        width: 74px;
}
.mobile .reduce-height .main-navigation-logo .main-navigation-logo-img img {
        margin-top: 12px; /*JLMA*/
}
.desktop #menu-btnx { 
    margin-top: 20px;
    cursor: pointer;
}
.mobile #menu-btnx { 
    margin-top: 18px;
    cursor: pointer;
}


/** Menu **/
.menu-circle{
    height:25px;
    width:25px;
    background-color:transparent;
    border:1px solid #fff;
    border-radius:50%;
    -moz-border-radius: 50%;
    -webkit-border-radius: 50%;
    float:right;
    margin:20px 0px 0px 30px;
    transition: 1s;
    cursor: pointer;
}
@media (min-width:992px) {
    .menu-circle:hover{
        background-color:#fff;
        transition:.5s;
        text-decoration:none;
    }
    .menu-circle:active{
        background-color:#fff;
        text-decoration:none;
    }
}
/** MENU MOBILE **/
.mmmenu{
    position: fixed;
    width: 100%;
    background-color: #121212;
}
.mmmenu {
    position: fixed;
    /*display: none;*/
/*    width: -webkit-calc(100% - 50px);
    width: calc(100% - 50px);*/
    height: 0px;
    width: 100%;
/*    top: 85px;*/
    /*left: 50%;*/
    /*bottom: 55px;*/
/*    -webkit-transform: translateX(-50%);
    -ms-transform: translateX(-50%);
    transform: translateX(-50%);*/
/*    -webkit-transition: opacity 300ms cubic-bezier(0.77, 0, 0.175, 1);
    -moz-transition: opacity 300ms cubic-bezier(0.77, 0, 0.175, 1);
    -ms-transition: opacity 300ms cubic-bezier(0.77, 0, 0.175, 1);
    -o-transition: opacity 300ms cubic-bezier(0.77, 0, 0.175, 1);
    transition: opacity 300ms cubic-bezier(0.77, 0, 0.175, 1);*/
    z-index: 2;
}
.center-container {
    position: absolute;
    display: table;
    table-layout: fixed;
    width: 100%;
    height: calc(100% - 25px);
    background: none;
}
.home .mmmenu{
    top: 0;
}
.home .center-container{
    position: fixed;
    height: 100%;
}
.center-block {
    display: block;
    margin-right: auto;
    margin-left: auto;
}
.center-block {
    display: table-cell !important;
    vertical-align: bottom;
}
.menu {
    margin: 8% auto;
    transform: translateX(5%);
}
.menu-item {
    opacity: 0;
    font-size: 29px;
    text-transform: uppercase;
     outline: none;
    -webkit-font-smoothing: antialiased;
    -moz-osx-font-smoothing: grayscale;
}
/** END MOBILE **/

.menu-circle:focus{
    outline:none !important;
    text-decoration:none;
}
.menu-circle-active{
    background-color:#fff;
    text-decoration:none;
}
.contact-circle{
    height:25px;
    width:25px;
    background-color:transparent;
    border:1px solid #3BEAD0;
    border-radius:50%;
    margin:20px 0px 0px 0;
}
.contact-circle:hover{
    background-color:#3BEAD0;
    transition: 1s;
    text-decoration:none;
}
.contact-circle:active{
    background-color:#3BEAD0;
    text-decoration:none;
}
.contact-circle:focus{
    outline:none !important;
    text-decoration:none;
}
.contact-circle-active{
    background-color:#3BEAD0;
    text-decoration:none;
}
.contact-text{
    color: #fff;
    display: none;
    transition: 1s;
    font-family: 'BergenText-Regular', sans-serif;
    text-align: center;
    margin-top: 33px;
    font-size: 14pt;
    margin-left: -9px;

}

.about-circle{
    height:25px;
    width:25px;
    background-color:transparent;
    border:1px solid #F00B57;
    border-radius:50%;
    float:right;
    margin:20px 0px 0px 0;
}
.about-circle:hover{
    background-color:#F00B57;
    transition: 1s;
    text-decoration:none;
}
.about-circle:active{
    background-color:#F00B57;
    text-decoration:none;
}
.about-circle:focus{
    outline:none !important;
    text-decoration:none;
}
.about-circle-active{
    background-color:#F00B57;
    text-decoration:none;
}
.about-text {
    color: #fff;
    display: none;
    transition: 1s;
    font-family: 'BergenText-Regular', sans-serif;
    text-align: center;
    margin-top: 33px;
    font-size: 14pt;
}
.work-circle{
    height:25px;
    width:25px;
    background-color:transparent;
    border:1px solid #FBAE17;
    border-radius:50%;
    float:right;
    margin:20px 0px 0px 0;
}
.work-circle:hover{
    background-color:#FBAE17;
    text-decoration:none;
    transition: 1s;
}
.work-circle:active{
    background-color:#FBAE17;
    text-decoration:none;	
}
.work-circle:focus{
    outline:none !important;
    text-decoration:none;
}
.work-circle-active{
    background-color:#FBAE17;
    text-decoration:none;	
}
.work-text{
    color: #fff;
    display: none;
    transition: 1s;
    font-family: 'BergenText-Regular', sans-serif;
    text-align: center;
    margin-top: 33px;
    font-size: 14pt;
}
.team-circle{
    height:25px;
    width:25px;
    background-color:transparent;
    border:1px solid #151BBD;
    border-radius:50%;
    float:right;
    margin:20px 0 0 0
}
.team-circle:hover{
    background-color:#151BBD;
    transition: 1s;
    text-decoration:none;
}
.team-circle:active{
    background-color:#151BBD;
    text-decoration:none;
}
.team-circle:focus{
    outline:none !important;
    text-decoration:none;
}
.team-circle-active{
    background-color:#151BBD;
    text-decoration:none;
}
.reduce-height .all-menu span {
    color: #fff;
}
.all-menu span{
    color: #111111;
    visibility: hidden;
    opacity: 0;
    display: block;
    transition: 1s;
    font-family: 'BergenText-Regular', sans-serif;
    text-align: center;
    margin-top: 30px;
    position: relative;
    width: 100%;
    font-size: 15px;
    text-transform: uppercase;
}
.contact-circle span {
    margin-left: -12px;
}
.team-circle span {
    margin-left: -4px;
}
.work-circle span {
    margin-left: -4px;
}
.about-circle span {
    margin-left: -8px;
}
.all-menu a{
    position: absolute;
}
.all-menu a:hover span {
    visibility: visible;
    opacity: 1;
}
/** End Menu**/

/** Menu **/
.all-menu {
    position: absolute;
    top: 0;
    visibility: hidden;
/*    right: 25px;*/
    left: -20px;
}

.menu-circle{
    height:25px;
    width:25px;
    background-color:transparent;
    border:1px solid #fff;
    border-radius:50%;
    -moz-border-radius: 50%;
    -webkit-border-radius: 50%;
    float:right;
    margin:20px 0px 0px 30px;
    transition: 1s;
    cursor: pointer;
}
@media (min-width:992px) {
    .menu-circle:hover{
        background-color:#fff;
        transition:.5s;
        text-decoration:none;
    }
    .menu-circle:active{
        background-color:#fff;
        text-decoration:none;
    }
}
/** MENU MOBILE **/
.mmmenu{
    position: fixed;
    width: 100%;
    background-color: #121212;
}
.mmmenu {
    position: fixed;
    /*display: none;*/
/*    width: -webkit-calc(100% - 50px);
    width: calc(100% - 50px);*/
    height: 0px;
    width: 100%;
    top: 0;
/*    top: 85px;*/
    /*left: 50%;*/
    /*bottom: 55px;*/
/*    -webkit-transform: translateX(-50%);
    -ms-transform: translateX(-50%);
    transform: translateX(-50%);*/
/*    -webkit-transition: opacity 300ms cubic-bezier(0.77, 0, 0.175, 1);
    -moz-transition: opacity 300ms cubic-bezier(0.77, 0, 0.175, 1);
    -ms-transition: opacity 300ms cubic-bezier(0.77, 0, 0.175, 1);
    -o-transition: opacity 300ms cubic-bezier(0.77, 0, 0.175, 1);
    transition: opacity 300ms cubic-bezier(0.77, 0, 0.175, 1);*/
    z-index: 2;
}
.home .center-container {
    position: absolute;
    display: table;
    table-layout: fixed;
    width: 100%;
    height: calc(100% - 85px);
    background: none;
}
.home .mmmenu{
    top: 0;
}
.home .center-container{
    position: fixed;
    height: 100%;
}
.home .center-block {
    display: block;
    margin-right: auto;
    margin-left: auto;
}
.home .center-block {
    display: table-cell !important;
    vertical-align: bottom;
}
.menu {
    margin: 8% auto;
    transform: translateX(5%);
    padding: 0;
    list-style: none;
}
.menu-item {
    opacity: 0;
    line-height: 1.2;
    font-size: 29px;
    text-transform: uppercase;
    font-family: 'BergenText-Regular', sans-serif;
}
/** END MOBILE **/

.menu-circle:focus{
    outline:none !important;
    text-decoration:none;
}
.menu-circle-active{
    background-color:#fff;
    text-decoration:none;
}
.contact-circle{
    height:25px;
    width:25px;
    background-color:transparent;
    border:2px solid #3BEAD0;
    border-radius:50%;
    margin:20px 0px 0px 0;
}
.contact-circle:hover{
    background-color:#3BEAD0;
    transition: 1s;
    text-decoration:none;
}
.contact-circle:active{
    background-color:#3BEAD0;
    text-decoration:none;
}
.contact-circle:focus{
    outline:none !important;
    text-decoration:none;
}
.contact-circle-active{
    background-color:#3BEAD0;
    text-decoration:none;
}
.contact-text{
    color: #fff;
    display: none;
    transition: 1s;
    font-family: 'BergenText-Regular', sans-serif;
    text-align: center;
    margin-top: 33px;
    font-size: 14pt;
    margin-left: -9px;

}

.about-circle{
    height:25px;
    width:25px;
    background-color:transparent;
    border:2px solid #F00B57;
    border-radius:50%;
    float:right;
    margin:20px 0px 0px 0;
}
.about-circle:hover{
    background-color:#F00B57;
    transition: 1s;
    text-decoration:none;
}
.about-circle:active{
    background-color:#F00B57;
    text-decoration:none;
}
.about-circle:focus{
    outline:none !important;
    text-decoration:none;
}
.about-circle-active{
    background-color:#F00B57;
    text-decoration:none;
}
.about-text {
    color: #fff;
    display: none;
    transition: 1s;
    font-family: 'BergenText-Regular', sans-serif;
    text-align: center;
    margin-top: 33px;
    font-size: 14pt;
}
.work-circle{
    height:25px;
    width:25px;
    background-color:transparent;
    border:2px solid #FBAE17;
    border-radius:50%;
    float:right;
    margin:20px 0px 0px 0;
}
.work-circle:hover{
    background-color:#FBAE17;
    text-decoration:none;
    transition: 1s;
}
.work-circle:active{
    background-color:#FBAE17;
    text-decoration:none;	
}
.work-circle:focus{
    outline:none !important;
    text-decoration:none;
}
.work-circle-active{
    background-color:#FBAE17;
    text-decoration:none;	
}
.work-text{
    color: #fff;
    display: none;
    transition: 1s;
    font-family: 'BergenText-Regular', sans-serif;
    text-align: center;
    margin-top: 33px;
    font-size: 14pt;
}
.team-circle{
    height:25px;
    width:25px;
    background-color:transparent;
    border:2px solid #151BBD;
    border-radius:50%;
    float:right;
    margin:20px 0 0 0
}
.team-circle:hover{
    background-color:#151BBD;
    transition: 1s;
    text-decoration:none;
}
.team-circle:active{
    background-color:#151BBD;
    text-decoration:none;
}
.team-circle:focus{
    outline:none !important;
    text-decoration:none;
}
.team-circle-active{
    background-color:#151BBD;
    text-decoration:none;
}
.contact-circle span {
    margin-left: -12px;
}
.team-circle span {
    margin-left: -4px;
}
.work-circle span {
    margin-left: -4px;
}
.about-circle span {
    margin-left: -8px;
}
.all-menu a{
    position: absolute;
}
.all-menu a:hover span {
    visibility: visible;
    opacity: 1;
}
/** End Menu**/

.main-inner-navigation {
    position: relative;
    display: inline-block;
    float: right;
    margin-right: 25px;
}
.main-navigation-logo {
    float: left;
    margin-left: 2vw;
}

a.menu-link {
    margin-left: 0;
}
a.menu-link:link
{
   color: #ffffff;
   text-decoration: none;
}
a.menu-link:visited
{
   color: #ffffff;
   text-decoration: none;
}
a.menu-link:hover
{
   color: #ffffff;
   text-decoration: none;
}
a.menu-link:active
{
   color: #ffffff;
   text-decoration: none;
}


.main-navigation {
    position: relative;
    top: 0;
    left: 0;
    text-align: left;
    width: 100%;
    -webkit-transition: height 0.27s cubic-bezier(0.000, 0.000, 0.580, 1.000);
    -moz-transition: height 0.27s cubic-bezier(0.000, 0.000, 0.580, 1.000);
    -ms-transition: height 0.27s cubic-bezier(0.000, 0.000, 0.580, 1.000);
    -o-transition: height 0.27s cubic-bezier(0.000, 0.000, 0.580, 1.000);
    transition: height 0.27s cubic-bezier(0.000, 0.000, 0.580, 1.000);
    background-color: rgba(17, 17, 17, .5);
    z-index: 9;
}
.fix-sticky {
    position: fixed!important;
    top: 0;
    left: 0;
}

.fondo1 {
    background-image:url(../assets/team/img/About-capavilities_white.gif);
    background-repeat: no-repeat;
    /*background-attachment: fixed;*/
    background-position: center;
    background-size: contain;
    height: 714px;
}

.fondo2 {
    background-image:url(../assets/team/img/About-3Steps_Loop.gif);
    background-repeat: no-repeat;
    /*background-attachment: fixed;*/
    background-position: center;
    background-size: contain;
    height: 714px;
    z-index:1
}
.fondo3 {
    background-image:url(../assets/team/img/About-3Steps_Loop.gif);
    background-repeat: no-repeat;
    /*background-attachment: fixed;*/
    background-position: center;
    background-size: contain;
    height: 714px;
    width: 100%;
    display:none;
    z-index:1;
    position: absolute;
}
.fondo4 {
    background-image:url(../assets/team/img/3Stepstext.png);
    background-repeat: no-repeat;
    /*background-attachment: fixed;*/
    background-position: center;
    background-size: contain;
    height: 714px;
    display:none;
    z-index:1;
    position: relative;
}

.footers a {
    color: #111111;
    font-size: 18px;
}
.footers a:hover, .footers a:focus {
    color: #111111;
}

@media (max-width: 991px){
    .about-section-1 .about-row-1 {
        margin-top: 15%;
    }
}

#menu-btnx {
    cursor: pointer;
}
/*#menu-btnx:hover path {
    fill: #111111;
    transition: fill .4s ease;
}
.reduce-height #menu-btnx:hover path {
    fill: #fff;
    transition: 200ms;
    transition-timing-function: ease-in-out;
    -webkit-transition: 200ms;
    -webkit-transition-timing-function: ease-in-out;
}*/
.opened.mobile .reduce-height #menu-btnx path{
    fill: #fff;
    transition: 200ms;
    transition-timing-function: ease-in-out;
    -webkit-transition: 200ms;
    -webkit-transition-timing-function: ease-in-out;
}

.reduce-height #menu-btnx.active path {
    fill: #fff;
    transition: fill .4s ease;
}
.reduce-height #menu-btnx .cls-x {
    stroke: #fff;
    transition: fill .4s ease;
}
#menu-btnx.active path {
    fill: #111111;
    transition: fill .4s ease;
}

div.caja::before {
    content: "";
    border-bottom: 2px solid #111111;
    position: absolute;
    left: 0;
    top: 0;
    height: 1px;
    width: 38%;
}
div.caja span {
    margin: 0 auto;
    width: 250px;
    height: auto;
    display: block;
    position: relative;
    top: -28px;
    color: #111111;
    text-align: center;
    font-family: 'BergenText-Bold', sans-serif;
}

@media (max-width:992px) {
    .loader {
        background-size: 40%;
    }
}

.opened #grid-logo .cls-1, .opened #grid-logo .cls-2 {
    fill: #fff;
}
.opened #menu-btnx .cls-x{
    stroke: #fff;
    fill: #fff;
}
.mobile.opened {
    overflow: hidden;
}

.contact.mobile .change .cls-x{
    stroke: #ffffff;
}
.contact.mobile .cls-1, .contact.mobile .cls-2 {
    fill: #ffffff;
}

.mobile h3.text-menu-link {
    position: relative;
    display: inline-block;
    line-height: 1.2;
    margin: 0;
    font-size: 29px;
    text-transform: uppercase;
    text-decoration: none;
    color: #fff;
    z-index: 1;
}
.mobile .text-menu-link:before,
.mobile .text-menu-link:after {
    content: attr(data-content);
    position: absolute;
    top: 0;
    left: 0;
    right: 0;
    bottom: 0;
}
.mobile .text-menu-link:before {
    color: #00ffff;
    z-index: -1;
}
.mobile .text-menu-link:after {
    color: #ff00ff;
    z-index: -2;
}

.mobile .menu-link:hover .text-menu-link:before,
.mobile .menu-link.active .text-menu-link:before,
.mobile .text-menu-link:hover:before {
    -webkit-animation: glitchy 0.3s ease 0.3s infinite;
    animation: glitchy 0.3s ease 0.3s infinite;
}

.mobile .menu-link:hover .text-menu-link:after,
.mobile .menu-link.active .text-menu-link:after,
.mobile .text-menu-link:hover:after {
    animation: glitchy 0.3s ease infinite reverse;
}
.about__title{
        font-weight: 900;
    font-size: 72px;
    text-transform: uppercase;
}
.about__copy{
    font-size: 16px;
    font-family: MarkPro;
    line-height: 1.5em;
}
@-webkit-keyframes glitchy {
    0% {
        -webkit-transform: translate(-2px, 2px);
        transform: translate(-2px, 2px);
    }
    25% {
        -webkit-transform: translate(-2px, -2px);
        transform: translate(-2px, -2px);
    }
    50% {
        -webkit-transform: translate(2px, 2px);
        transform: translate(2px, 2px);
    }
    75% {
        -webkit-transform: translate(2px, -2px);
        transform: translate(2px, -2px);
    }
    100% {
        -webkit-transform: translate(-2px, 2px);
        transform: translate(-2px, 2px);
    }
}
@keyframes glitchy {
    0% {
        -webkit-transform: translate(-2px, 2px);
        transform: translate(-2px, 2px);
    }
    25% {
        -webkit-transform: translate(-2px, -2px);
        transform: translate(-2px, -2px);
    }
    50% {
        -webkit-transform: translate(2px, 2px);
        transform: translate(2px, 2px);
    }
    75% {
        -webkit-transform: translate(2px, -2px);
        transform: translate(2px, -2px);
    }
    100% {
        -webkit-transform: translate(-2px, 2px);
        transform: translate(-2px, 2px);
    }
}
    @media (max-width: 680px){
        .service-cols {
            margin-top: -150px;
        }
    }
    
    @media (min-width: 1480px){
        .service-cols {
            width: 1480px;
            padding-left: 50px;
        }
    }
    @media (max-width: 500px){
        .about__title{
            font-size: 50px!important;
        }
        .about__copy{
            font-size: 16px!important;
            line-height: 1.5em!important;
        }
        .about-row-1{
            padding:45px 0px 0;
        }
        .fondo1[data-v-039c5b43] {
            height: 294px;
        }
        .service-cols ul{
            padding: 0;
        }
        .service-cols .col-xs-12{
            padding: 0;
        }
        .fondo2{
            padding: 0;
            margin: 0;
            height: 320px;
            background-position: top;
        }
        .about-section-2 .container{
            padding-right: 20px;
            padding-left: 20px;
        }
        .about-section-2 {
            padding-top: 30%;
        }
        .about-row-1.row .col-md-9, .about-row-1.row .col-md-12{
            padding: 0px!important;
        }
        .about-section-2 .container, .about-section-2 .container-fluid {
            width: 90%!important;
        }
    }
</style>
<script>
import AOS from "aos";

export default {
    components: {

    },
    data() {
        return {
            name: '',
            company: ''
        };
    },
    methods: {
        animateLoader(){
            // First Animation Batch
            document.querySelector('.loader__screen').classList.add('start__animation');
            document.querySelector('.loader__screen .right__bar').classList.add('first__animation__sideblock');
            //document.querySelector('.loader__wrapper').classList.add('first__loader__gif');
        },
        removeLoader(){
            // Second Animation Batch
            document.querySelector('.loader__screen').classList.remove('start__animation');
            document.querySelector('.loader__screen').classList.add('end__loader');
AOS.init({disable: 'mobile',anchorPlacement: 'top-bottom',});        },
        formSubmit(e) {
            e.preventDefault();
            var submitButton = document.querySelector('.btn');

            var currentStep = submitButton.dataset['step'];
            if(currentStep === '1'){
                var stepOneInputs = [...document.querySelectorAll('.step_one input')];
                stepOneInputs.forEach(input => {
                    if(input.value.length >= 3){
                        input.classList.remove('error__input');
                        input.parentElement.classList.remove('label__error');
                    }else{
                        input.classList.add('error__input');
                        input.parentElement.classList.add('label__error');
                    }
                });

                if([...document.querySelectorAll('.error__input')].length < 1){
                    submitButton.dataset['step'] = 2;
                    document.querySelector('.step_one').classList.add('fadeOutUp');
                    document.querySelector('.step_two').classList.add('fadeInUp');
                }

            }else if(currentStep === '2'){
                var stepTwoInputs = [...document.querySelectorAll('.step_two input')];
                stepTwoInputs.forEach(input => {
                    if(input.value.length >= 3){
                        input.classList.remove('error__input');
                        input.parentElement.classList.remove('label__error');
                    }else{
                        input.classList.add('error__input');
                        input.parentElement.classList.add('label__error');
                    }
                });

                if([...document.querySelectorAll('.error__input')].length < 1){


                    let currentObj = this;
                    this.axios.post('http://raxo-form.test/form.php', {
                        name: this.name,
                        company: this.company
                    })
                    .then(function (response) {
                        document.querySelector('.step_two').classList.remove('fadeInUp');
                        document.querySelector('.step_two').classList.add('fadeOutUp');
                        document.querySelector('.form__message').classList.add('fadeInUp');
                        submitButton.dataset['step'] = 3;
                        submitButton.innerHTML = "Repeat";
                    })
                    .catch(function (error) {
                        currentObj.output = error;
                        console.log('We cannot connect to the database :( try again later');
                    });
                }
            }else if(currentStep === '3'){
                submitButton.dataset['step'] = 1;
                document.querySelector('.step_one').classList.remove('fadeInUp');
                document.querySelector('.step_one').classList.remove('fadeOutUp');
                document.querySelector('.step_two').classList.remove('fadeInUp');
                document.querySelector('.step_two').classList.remove('fadeOutUp');
                document.querySelector('form').classList.remove('fadeInUp');
                document.querySelector('form').classList.remove('fadeOutUp');
                document.querySelector('.form__message').classList.remove('fadeInUp');
                document.querySelector('.form__message').classList.remove('fadeOutUp');
                var allInputs = [...document.querySelectorAll('input')];
                allInputs.forEach(input => {
                    input.value = "";
                });
            }
            else{
                alert('Something goes wrong, please try again in a couple of minutes');
            }


        }
    },
     mounted(){
        document.querySelector('.loader__screen').style.display = 'none';

        var logo = [...document.querySelectorAll('.logo')];
        var menu = [...document.querySelectorAll('.menu__icon')]; 
        var bottomLinks = [...document.querySelectorAll('.bottom__links a')]; 
        var listElements = [...document.querySelectorAll('.nav__links a')]; 
        var buttons = [...document.querySelectorAll('.lc button')]; 
        var aLinks = [...document.querySelectorAll('a')]; 
        var otherElements = [...document.querySelectorAll('.team-overlay')]; 
        var allElements = logo.concat(menu).concat(bottomLinks).concat(listElements).concat(buttons).concat(aLinks).concat(otherElements);        

        allElements.forEach(function(element) {
        element.addEventListener("mouseenter", function() {
            var theCursor = document.querySelector('.cursor')
            theCursor.classList.add('morphed--cursor');
        });
        element.addEventListener("mouseleave", function() {
            var theCursor = document.querySelector('.cursor')
            theCursor.classList.remove('morphed--cursor');
        });
        })

        this.animateLoader();
        setTimeout(function() { 
            this.removeLoader();
        }.bind(this), 
        300);
        // Add white color classes to the cursor and the menu icon
        document.querySelector('.cursor').classList.add('white-cursor');
        document.querySelector('.menu__icon').classList.add('white-menu');

        [...document.querySelectorAll('.logo__black')][0].classList.remove('show__logo');
        [...document.querySelectorAll('.logo__white')][0].classList.add('show__logo');

        // Close navbar clicking outside
        document.querySelector('.section__wrapper').addEventListener('click', function(){
            if(document.querySelector('.openedDesktop')){
                document.querySelector('.sidenav').classList.remove('openedDesktop');
                document.querySelector('.section__wrapper').classList.remove('active');
                document.body.style.overflow = 'auto';
            }
        })
    },

};
</script>
