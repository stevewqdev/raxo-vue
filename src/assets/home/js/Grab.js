...
class Grab {
  constructor({
    ...
    let mainEl = document.querySelector(".imaginary__frame");
    mainEl.addEventListener("mousedown", this.onMouseDown.bind(this));
    this.listen("mousemove", this.onMouseMove.bind(this));
    this.listen("mouseup", this.onMouseUp.bind(this));
    if (window.screen.width < 768) {
      mainEl.addEventListener("click", this.onMouseDown.bind(this), {passive: true});
    }
    this.listen("touchmove", this.onMouseMove.bind(this), {passive: true});
    this.listen(["touchend", "touchcancel"], this.onMouseUp.bind(this), {passive: true});
  }
